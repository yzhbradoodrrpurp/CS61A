...
def boar_brawl(player_score, opponent_score):
    ...
    boar = 3 * abs(ones_digit - tens_digit)
...
def sus_points(score):
    ...
    sus = (num_factors(score) == 3) or (num_factors(score) == 4)

    if sus:
        for sus_score in range(score + 1, 2 * score + 1):
            if is_prime(sus_score):
                return sus_score
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(0, goal + 1):
        for opponent_score in range(0, goal + 1):
            if initial != strategy(score, opponent_score):
                return False
...
def make_averaged(original_function, times_called=1000):
    ...
    def averaged_dice(*args):
        ...
        for i in range(times_called):
            summary += original_function(*args)

        average_score = summary / times_called
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if sus_strategy(score, opponent_score) == 0 or boar_strategy(score, opponent_score) == 0:
        return 0
