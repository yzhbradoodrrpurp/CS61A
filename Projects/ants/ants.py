...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place_to_search = self.place
        current_bound = 0

        while True:
            if self.lower_bound <= current_bound <= self.upper_bound:
                if place_to_search.is_hive:
                    return None
                if place_to_search.bees:
                    return random_bee(place_to_search.bees)

            if place_to_search.entrance is not None:
                current_bound += 1
                place_to_search = place_to_search.entrance
            else:
                return None
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if not is_chewing and self.place.bees:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # No damage change to the queen itself.
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 1
        "*** YOUR CODE HERE ***"
        # Re-assign functions and methods to control the behavior of 'target.action'.
        if target:
            target.slow_turns = 5
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # is_slowed = hasattr(self, 'slow_turns') and self.slow_turns > 0
        # if is_slowed and gamestate.time % 2 == 1:
        #     return
        #
        # if self.scared_turns > 0:
        #     if self.blocked():
        #         self.sting(self.place.ant)
        #     elif not self.place.entrance.is_hive and self.place.entrance is not None:
