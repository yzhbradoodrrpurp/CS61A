    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and':  # and expressions
            ...
            if exp.first in OPERATORS:
                return calc_apply(OPERATORS[exp.first], exp.rest)  # UPDATE THIS FOR Q2, what is type(operator)?
            elif exp.first in bindings:
                return calc_apply(bindings[exp.first], exp.rest)
...
def floor_div(args):
    ...
    while args.rest is not nil:
        ...
        elif isinstance(args.rest.first, Pair):
            divisor = calc_eval(args.rest.first)
            result //= divisor
        elif isinstance(args.rest.first, float):
            result //= args.rest.first
...
def eval_define(expressions):
    ...
    elif isinstance(expressions.rest.first, str):
        if expressions.rest.first in bindings.keys():
            bindings[expressions.first] = bindings[expressions.rest.first]
        elif expressions.rest.first in OPERATORS.keys():
            bindings[expressions.first] = OPERATORS[expressions.rest.first]
    elif isinstance(expressions.rest.first, int):
        bindings[expressions.first] = expressions.rest.first
    elif isinstance(expressions.rest.first, float):
        bindings[expressions.first] = expressions.rest.first
